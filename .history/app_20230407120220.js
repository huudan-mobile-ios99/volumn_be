  ...
  promotionHistoryModel.find(function (err, data) {
    if (err) {
      ...
      if (data == null || data.length === 0) {
        ...
        sheet.addRow(["STT", "Number Customer","Name Customer","Name Promotion","Answer","IsCorrect"]); // Add header row
        data.forEach((item, index) => {
          console.log(item)
          sheet.addRow([index,item.number_customer]); // Add data rows
        });
        
        sheet.getRow(1).height = 40; // Set row height of row 1 to 40
        sheet.getColumn(1).width = 15; // Set column width of column A to 15
        sheet.getColumn(2).width = 10; // Set column width of column B to 10
