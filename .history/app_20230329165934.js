...
app.get("/file/:filename", async (req, res) => {
    const photoModel =config.collection_photo;
    // const photoModel =config.clientConnect.collection('photos.files');
    // try {
    //     // const file = await photoModel.findOne({ filename: req.params.filename },  async function (err, data) {
    //     //     if(err){
    //     //         res.send(err)
    //     //     }else 
    //     //     res.send(data)
    //     // });
    //     // const readStream = photoModel.createReadStream(file.filename);
    //     // readStream.pipe(res);
    //     // res.send(req.params.filename)
    //     // await photoModel.find()
    // } catch (error) {
    //     res.send(`error, not found ${req.params.filename}`);
    // }
photoModel.findOne({ filename: req.params.filename }, async function (err, data) {
            if (err) {
                console.log(err);
            }
            else {
