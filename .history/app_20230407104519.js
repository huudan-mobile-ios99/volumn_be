  ...
  promotionHistoryModel.find(function (err, data) {
    if (err) {
      ...
      if (data == null || data.length == 0) {
        ...
        const excelFilePath = path.join(process.cwd(), excelFileName); // Use an absolute path for the file path
        XLSX.writeFile(workbook, excelFilePath);
        res.download(excelFilePath, excelFileName, (err) => {
          if (err) throw err;
          // Clean up the generated Excel file after it's downloaded
          fs.unlinkSync(excelFilePath);
          console.log(`Excel file was located at: ${excelFilePath}`); // Log the file location
        });
        res.send({ "status": false, "message": " promotion history found", "totalResult": data.length, "data": null });
