...
app.get('/file/:filename', async (req,res)=>{
    const collection = await config.getCollectionPhoto();
    collection.findOne({ filename: req.params.filename }, async function (err, data) {
            if (err) {
                console.log(err);
            }
            else {
                if (data != null) {
                    res.send({ "status": false, "message": "fail get download file image promotion", "data": null });
                } else {
