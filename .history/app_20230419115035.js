    ...
    loginModel.findOne({ name: req.body.name, number: req.body.number }, async function (err, data) {
        if (err) {
            ...
            if (data == null || data.length == 0) {
                ...
                const result = loginModel.updateOne(
                    { name:name number: data.number },
                    { $set: { loginCount: data.loginCount + 1, lastUpdate: data.newUpdate } }
                )
                if (result.acknowledged) {
                    res.json({ success: true, message: 'Login updated successfully' });
                } else {
                    res.json({ success: false, message: 'Login not found' });
                }
            }
        }
    });
