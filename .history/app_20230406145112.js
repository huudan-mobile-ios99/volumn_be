...
app.delete('/delete_file:filename', async (req, res) => {
    const filename = req.params.filename;
    try {
        config.bucketGlobal.delete({filename: filename},(err)=>{
             if (err) {
        console.error(`Failed to delete file "${fileNameToDelete}": ${err}`);
        res.status(500).send(`Failed to delete file "${fileNameToDelete}"`);
      } else {
        console.log(`File "${fileNameToDelete}" deleted successfully`);
        res.status(200).send(`File "${fileNameToDelete}" deleted successfully`);
      }
        })
    } catch (error) {
        console.error(error);
