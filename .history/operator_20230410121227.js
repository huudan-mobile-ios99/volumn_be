...
static async createUserRecord(req, res) {
  const { number, loginCount } = req.body; // Extracting number and loginCount from request body

  // Validate request body
  if (!number || !loginCount || ) {
    return res.status(400).json({ error: 'number and loginCount are required' });
  }
