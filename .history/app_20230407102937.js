...
router.get('/list_promotion_history', async (req, res) => {
  promotionHistoryModel.find(function (err, data) {
    if (err) {
      console.log(err);
      ...
      if (data == null || data.length == 0) {
        ...
        res.download(excelFileName, (err) => {
          if (err) throw err;
          // Clean up the generated Excel file after it's downloaded
...
In this updated version, we have combined the functionality of fetching promotion history data using your existing Mongoose model (promotionHistoryModel) and exporting the fetched data to an Excel file using the "xlsx" library. The fetched data is first sent as a response with appropriate status, message, total result, and data. Then, the data is converted to an Excel sheet and appended to a new workbook. The resulting Excel file is then sent as a download response to the client using the "res.download" function. Finally, the generated Excel file is cleaned up using "fs.unlinkSync" after it's downloaded.
