    ...
    loginModel.findOne({ name: req.body.name, number: req.body.number }, async function (err, data) {
        if (err) {
            ...
            if (data == null || data.length == 0) {
                ...
                const filter = { name: name, number: number };
                const update = { $set: { loginCount: 20, lastUpdate: data.newUpdate, name: name, number: number } };
                const options = { upsert: false };
                loginModel.updateOne(
                    {
                        id: data._id,
                        loginCount: data.loginCount + 1,
                        lastUpdate: data.newUpdate,
                    ...
                    , async function (err, data2) {
                        if (err) {
                            ...
                            if (!data2) {
                                ...
                                res.send({ status: true, message: "update promotion success", data: data.loginCount+1 });
                            }
                        }
                    });
            }
