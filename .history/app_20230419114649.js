    ...
    loginModel.findOne({ name: req.body.name, number: req.body.number }, async function (err, data) {
        if (err) {
            ...
            if (data == null || data.length == 0) {
                ...
                res.send({ "status": true, "message": "find list user record success - can not create because of duplicate ", "totalResult": data.length, "data": data.number });
                // loginModel.updateOne(
                //     { name: name, number: number },
                //     { $set: { loginCount: 5, lastUpdate: new Date() } }
                // )
            }
        }
    });
