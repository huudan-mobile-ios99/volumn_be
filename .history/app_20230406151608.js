...
app.delete('/delete_file/:filename', async (req, res) => {
    ...
    res.json(file)
if (!file) {
        console.error(`File "${fileNameToDelete}" not found`);
        return res.status(404).send(`File "${fileNameToDelete}" not found`);
      }

      // Delete the file from the database
      await bucket.delete(file._id);
      console.log(`File "${fileNameToDelete}" deleted successfully`);
      return res.status(200).send(`File "${fileNameToDelete}" deleted successfully`);
