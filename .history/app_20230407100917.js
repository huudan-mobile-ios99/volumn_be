...
app.get('/export_promotion_history', async (req, res) => {
  try {
    ...
    const data = promotionHistoryModel.find(function (err, data) {
        if (err) {
            console.log(err);
        }
        else {
    ...
    const sheetName = 'Sheet_Promotion_History';

    // Convert data to Excel sheet format
    const jsonData = data.map((item) => ({
      // Customize this part based on your promotion history data structure
      id: item.id,
      title: item.title,
    ...
    // Write the Excel workbook to a temporary file
    const tempFilePath = path.join(__dirname, 'temp', 'promotion_history.xlsx');
    fs.writeFileSync(tempFilePath, XLSX.write(workbook, { type: 'buffer', bookType: 'xlsx' }));

    // Send the Excel file as a response for download
    res.setHeader('Content-Disposition', 'attachment; filename=promotion_history.xlsx');
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.sendFile(tempFilePath);
  } catch (err) {
    console.error('Failed to fetch data and export to Excel:', err);
    res.status(500).json({ error: 'Failed to fetch data and export to Excel' });
  }
});