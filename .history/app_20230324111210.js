...
app.put('/update_promotion_status', async (req, res) => {
    const message = 'updated promotion status';
        ...
        try {
             await promotionModel.findOneAndUpdate({ id: promotion.id }, promotion, { new: false },{returnOriginal: false });
            if (!data) {
                          return  res.status(404).send({ message: `${message} with id: ${req.body.id}` })
                        } else {
                           return res.send({"message":`some message ${promotion.id}`});
                        }
        } catch (err) {
