...
// Loop through all rows and columns to find maximum text lengths
sheet.eachRow((row, rowNumber) => {
  row.eachCell((cell, colNumber) => {
    const cellValue = cell.value ? cell.value.toString() : '';
    maxTextLengths[colNumber - 1] = Math.max(maxTextLengths[colNumber - 1], cellValue.length);
  });
});

// Set column widths based on maximum text lengths
maxTextLengths.forEach((maxTextLength, index) => {
  sheet.getColumn(columnHeaders[index]).width = Math.max(10, maxTextLength + 2);
});
