...
app.put('/update_promotion_status', async (req, res) => {
    const message = 'updated promotion status';
        ...
        promotionModel.findOne({ id: req.body.id }, async function (err, data) {
            if (err) {
                console.log(err);
            }
            else {
                if (data != null) {
                    return res.send({ "status": false, "message": message, "data": 'fail to find promotion by id' });
                } else {
                    
                   return res.send({ "status": true, "message": message, "data": promotion });
                }
            }
        });
});
