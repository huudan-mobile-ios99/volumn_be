...
app.put('/update_promotion_status', async (req, res) => {
    ...
    try {
        ...
        try {
    const promo = await promotionModel.findOneAndUpdate({ id: promotion.id },promotion, { new: true });
    res.json(promo);
  } catch (err) {
...
app.post('/create_promotion_history', async (req, res) => {
    const promotion = new promotionHistoryModel(req.body);
    const message = 'created new promotion history';
    try {
        promotionHistoryModel.findOne({ id: promotion.id }, async function (err, data) {
            if (err) {
                console.log(err);
            }
            else {
