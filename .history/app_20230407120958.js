  ...
  promotionHistoryModel.find(function (err, data) {
    if (err) {
      ...
      if (data == null || data.length === 0) {
        ...
        // Set column width based on maximum text length in column A
const maxTextLength = sheet.getColumn(1).values.reduce((acc, row) => {
  ...
  return Math.max(acc, cellValue.length);
        ...
        sheet.getColumn(0).width = Math.max(10, maxTextLength + 2);// Set column width of column B to 10
        sheet.getColumn(1).width = Math.max(10, maxTextLength + 2);// Set column width of column B to 10
        sheet.getColumn(2).width = Math.max(10, maxTextLength + 2);// Set column width of column B to 10
        sheet.getColumn(3).width = Math.max(10, maxTextLength + 2);// Set column width of column B to 10
        sheet.getColumn(4).width = Math.max(10, maxTextLength + 2);// Set column width of column B to 10
        sheet.getColumn(5).width = Math.max(10, maxTextLength + 2);// Set column width of column B to 10
        sheet.getColumn(6).width = Math.max(10, maxTextLength + 2);// Set column width of column B to 10
