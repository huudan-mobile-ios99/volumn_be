  ...
  promotionHistoryModel.find(function (err, data) {
    if (err) {
      ...
      if (data == null || data.length === 0) {
        ...
        // Add data rows up to the 7th item and set background color to light green if text value is not empty
for (let i = 0; i < 7 && i < data.length; i++) {
  const item = data[i];
  console.log(item);
  const row = sheet.addRow([i+1,item.number_customer,item.name_customer,item.name_promotion,item.answer,item.isCorrect,item.dateTime]); // Add data rows
  if (item.number_customer || item.name_customer || item.name_promotion || item.answer || item.isCorrect || item.dateTime) {
    row.fill = {
      type: 'pattern',
      pattern: 'solid',
