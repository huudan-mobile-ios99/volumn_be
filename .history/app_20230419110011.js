...
const loginModel = requore('./model/login')
...
app.post("/list_user_record",  async (req, res) => {
    loginModel.find({ name: req.body.name,number:req.body.number }, async function (err, data) {
        if (err) {
            console.log(err);
        }
        else {
            if (data == null || data.length == 0) {
                ...
                res.send({ "status": true, "message": "find list user record active success", "totalResult": data.length, "data": data });
            }
        }
    });
})
