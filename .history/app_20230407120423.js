    ...
    if (err) {
      ...
      if (data == null || data.length === 0) {
        ...
        data.forEach((item, index) => {
          console.log(item)
          sheet.addRow([index,item.number_customer,item.name_customer,item.name_promotion,item.answer,item.isCorrect,item.dateTime]); // Add data rows
        });
        
        sheet.getRow(1).height = 25; // Set row height of row 1 to 40
        sheet.getColumn(1).width = 25; // Set column width of column A to 15
        sheet.getColumn(2).width = 25; // Set column width of column B to 10
        sheet.getColumn(3).width = 50; // Set column width of column B to 10
        sheet.getColumn(4).width = 50; // Set column width of column B to 10
        sheet.getColumn(5).width = 50; // Set column width of column B to 10
        sheet.getColumn(6).width = 50; // Set column width of column B to 10
