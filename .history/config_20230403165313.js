...
let bucket;
const bucketGlobal = new GridFSBucket(connection, {
...
async function getFileByName(filename) {
  ...
  try {
    ...
    const { contentType, metadata } = file;
    const metadataStream = bucketGlobal.openDownloadStream(metadata._id);
    metadataStream.on('data', (chunk) => {
      file.metadata = JSON.parse(chunk.toString());
      file.contentType = contentType;
    });

    metadataStream.on('error', (error) => {
      console.error(`Error while getting metadata for file ${filename}: ${error}`);
    });
    ...
    console.error(`Error while getting file ${filename}: ${error}`);
    return undefined;
  }
}
