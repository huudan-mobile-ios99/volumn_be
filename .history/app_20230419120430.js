    ...
    loginModel.findOne({ name: req.body.name, number: req.body.number }, async function (err, data) {
        if (err) {
            ...
            if (data == null || data.length == 0) {
                ...
                const update = { $set: { loginCount: data.loginCount + 1, lastUpdate: data.newUpdate,name:name,number:number } };
                const filter = { name: name,number:number };
                const options = { upsert: true };
                loginModel.updateOne(filter, update, options, async function (err, data) {
                    if (err) {
                        console.log(err);
                    }
                    else {
